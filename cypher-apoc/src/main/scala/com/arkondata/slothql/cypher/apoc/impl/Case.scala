...
import com.arkondata.slothql.cypher.apoc.APOC
...
object Case {
  import com.arkondata.slothql.cypher.apoc.impl.{ Case => CaseApoc }
  ...
  object QuerySyntax {
    def applyImpl[A: c.WeakTypeTag, R: c.WeakTypeTag](c: whitebox.Context)(f: c.Tree): c.Tree = {
      ...
      val resultTree = q"$f(_root_.com.arkondata.slothql.cypher.CypherFragment.Expr.Var[$A](${Literal(Constant(name))}))"
      q"_root_.com.arkondata.slothql.cypher.apoc.impl.Case.QuerySyntax.mk[$R](${c.prefix.tree}, $name, $resultTree)"
    }
