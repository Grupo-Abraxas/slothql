package com.arkondata.slothql.arrow
...
import com.arkondata.slothql.arrow.util.{ IsId, NotId }
import com.arkondata.slothql.util.ShapelessUtils
...
object Arrow {
  ...
  object SplitOps {
    def splitImpl[F: c.WeakTypeTag](c: whitebox.Context)(arrows: c.Tree*): c.Tree = {
      ...
      q"""
        _root_.com.arkondata.slothql.arrow.Arrow.Split(..${arrows.map(t => q"$t($idBuilder())")}) ∘ $f
      """
    }
  }
  ...
  object ChooseOps {
    ...
    def chooseImpl[F: c.WeakTypeTag](c: whitebox.Context)(alternatives: c.Tree*): c.Tree = {
      ...
      val choiceArrows = alternatives.map{ altBuilder =>
        q"$altBuilder(_root_.com.arkondata.slothql.arrow.Arrow.ChooseOps.AlternativeBuilder[$F])"
      }

      q"_root_.com.arkondata.slothql.arrow.Arrow.Choose[$F#Target].from(..$choiceArrows) ∘ $f"
    }
  }
