...
class CypherSyntaxWithMacros(override val c: blackbox.Context) extends CypherSyntaxPatternMacros(c) {
    ...
    val (returnTree, whereTree) = Opt.collect(ops) match {
      ...
      case Right((whereOpt, distinctOpt, skipOpt, limitOpt, orderSeq)) =>
        ...
        lazy val orderBy = orderSeq.map {
          case Opt.OrderByExpr(expr)     => q"_root_.scala.List(($expr, _root_.com.arkondata.slothql.newcypher.CypherFragment.Return.Order.Ascending))"
          case Opt.OrderByOrd(expr, ord) => q"_root_.scala.List(($expr, $ord))"
          case Opt.OrderBySel(expr, ord) => q"_root_.scala.List(($expr, $ord(_root_.com.arkondata.slothql.newcypher.CypherFragment.Return.Order)))"
        ...
        val ret = if (hasOptions) q"_root_.com.arkondata.slothql.newcypher.CypherFragment.Return.Options($return0, $distinct, $orderBy, $skip, $limit)"
                  else return0
  ...
  private val WithSyntax = c.mirror.staticModule("com.arkondata.slothql.newcypher.syntax.With")
}
