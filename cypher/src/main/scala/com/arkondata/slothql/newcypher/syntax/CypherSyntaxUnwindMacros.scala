    ...
    func.tree match {
      case Function(List(arg), body) =>
        ...
        c.Expr[CF.Query.Query0[R]](
          q"""
            val ${TermName(name)} = _root_.com.arkondata.slothql.newcypher.CypherFragment.Expr.Alias[${weakTypeOf[A]}]($nme)
            _root_.com.arkondata.slothql.newcypher.CypherFragment.Query.Clause(
              _root_.com.arkondata.slothql.newcypher.CypherFragment.Clause.Unwind($list, ${TermName(name)}),
