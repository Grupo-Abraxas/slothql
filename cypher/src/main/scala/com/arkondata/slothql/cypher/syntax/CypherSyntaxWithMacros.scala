...
class CypherSyntaxWithMacros(override val c: blackbox.Context) extends CypherSyntaxPatternMacros(c) {
  ...
  protected def withImpl[R](query: Tree, wildcard: Boolean, exprs: ExprWithInnerType*): c.Expr[CF.Query.Query0[R]] = {
    ...
    val (argNames, binds, returns) = exprs
      .zip(argNames0)
      .map { case ((tree, tpe), nme) =>
    ...
    val (returnTree, whereTree) = Opt.collect(ops) match {
      ...
      case Right((whereOpt, distinctOpt, skipOpt, limitOpt, orderSeq)) =>
        ...
        lazy val orderBy = orderSeq.map {
          case Opt.OrderByExpr(expr) =>
            q"_root_.scala.List(($expr, _root_.com.arkondata.slothql.cypher.CypherFragment.Return.Order.Ascending))"
          case Opt.OrderByOrd(expr, ord) => q"_root_.scala.List(($expr, $ord))"
          case Opt.OrderBySel(expr, ord) =>
            q"_root_.scala.List(($expr, $ord(_root_.com.arkondata.slothql.cypher.CypherFragment.Return.Order)))"
        ...
        val ret =
          if (hasOptions)
            q"_root_.com.arkondata.slothql.cypher.CypherFragment.Return.Options($return0, $distinct, $orderBy, $skip, $limit)"
          else return0
