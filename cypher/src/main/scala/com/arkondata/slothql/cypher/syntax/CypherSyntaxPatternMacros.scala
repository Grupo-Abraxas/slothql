...
class CypherSyntaxPatternMacros(val c: blackbox.Context) {
  ...
  def maybe[R: WeakTypeTag](opt: c.Expr[Boolean])(
    query: c.Expr[Node => CF.Query.Query0[R]]
  ): c.Expr[CF.Query.Query0[R]] = matchImpl(query, opt)
  ...
  protected def matchImpl[R: WeakTypeTag](
    query: c.Expr[Node => CF.Query.Query0[R]],
    optional: c.Expr[Boolean]
        ...
        case List(CaseDef(pattern, guard0, body)) =>
          ...
          val (rebind0, bindDefsNodeRel) = elems0.collect {
            case (Some(nme), Pattern.Tpe.Node, _) =>
              ...
              val tree =
                q"val ${TermName(name)} = _root_.com.arkondata.slothql.cypher.syntax.CypherSyntaxFromMacro.mkNode($nme)"
              (nme, name) -> tree
            case (Some(nme), Pattern.Tpe.Rel(dir), _) =>
              val name = c.freshName(nme)
              val tree =
                q"val ${TermName(name)} = _root_.com.arkondata.slothql.cypher.syntax.CypherSyntaxFromMacro.mkRel[$dir]($nme)"
              (nme, name) -> tree
          }.unzip
          val bindDefOptLet = letAliasOpt.map { nme =>
            val name = c.freshName(nme)
            val tree =
              q"val ${TermName(name)} = _root_.com.arkondata.slothql.cypher.syntax.CypherSyntaxFromMacro.mkPath($nme)"
            (nme, name) -> tree
          }
  ...
  private lazy val SyntaxPackageTypeSignature =
    c.mirror.staticPackage("com.arkondata.slothql.cypher.syntax").typeSignature

  private lazy val SyntaxIfGuardUnwrap =
    SyntaxPackageTypeSignature.decl(TermName("booleanCypherExprToBooleanForIfGuard"))

  private lazy val SyntaxIfGuardUnwrapOpt =
    SyntaxPackageTypeSignature.decl(TermName("optionalBooleanCypherExprToBooleanForIfGuard"))

  protected def qImpl[R: WeakTypeTag](
    query0: c.Expr[Node => CF.Query.Query0[R]]
  ...
  protected def maybeLetPattern(letOpt: Option[c.Expr[Alias]], elems: List[Pattern.Elem]): c.Expr[P] =
    letOpt
      .map(let => reify(P.Let(let.splice, mkPattern(elems).splice)))
  ...
  object PatternPlus {
    ...
    lazy val SyntaxUnapply_::= = rootMirror
      .staticModule("com.arkondata.slothql.cypher.syntax.$colon$colon$eq")
      .typeSignature
      .decl(TermName("unapply"))
  }
  ...
  object Pattern {
    ...
    object Elem {
      ...
      object Node {
        ...
        def unapply(elem: Elem): Option[(Option[String], c.Expr[Option[Alias]] => c.Expr[P.Node])] =
          PartialFunction.condOpt(elem) { case (a, Tpe.Node, e) =>
            a -> e.asInstanceOf[c.Expr[Option[Alias]] => c.Expr[P.Node]]
          }
      }

      object Rel {

        def unapply(elem: Elem): Option[(Option[String], Type, c.Expr[Option[Alias]] => c.Expr[P.Rel])] =
          PartialFunction.condOpt(elem) { case (a, Tpe.Rel(dir), e) =>
            (a, dir, e.asInstanceOf[c.Expr[Option[Alias]] => c.Expr[P.Rel]])
          }
      }
    }
    ...
    object Node {
      ...
      def unapply(tree: Tree): Option[Elem] = PartialFunction.condOpt(tree) {
        case pq"$obj0(${UnApply(m, args)})"
            if obj0.tpe <:< typeOf[ClassTag[syntax.Node]]
            && m.symbol == SyntaxNodeUnapplySeqSymbol =>
          (None, Tpe.Node, nodeExpr(args))
        case pq"$name@$obj0(${UnApply(m, args)})"
            if obj0.tpe <:< typeOf[ClassTag[syntax.Node]]
            && m.symbol == SyntaxNodeUnapplySeqSymbol =>
    ...
    object Rel {
      ...
      def unapply(tree: Tree): Option[Elem] = PartialFunction.condOpt(tree) {
        case pq"$obj0(${UnApply(m, args)})"
            if obj0.tpe <:< typeOf[ClassTag[syntax.Rel]]
    ...
    private def collectProps(args: Seq[Tree]): c.Expr[CollectedProps] = {
      ...
      if (paramProps0.size > 1)
        c.abort(c.enclosingPosition, "Graph element cannot have more than one Map expression parameter")
      ...
      paramProps
        .map(t => c.Expr[CollectedProps](q"_root_.scala.Right($t)"))
