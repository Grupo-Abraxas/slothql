...
import com.arkondata.slothql.cypher.CypherFragment.{ Clause, Expr, Known, Query, Return }
import com.arkondata.slothql.cypher.syntax.{ Match, ProcedureStringOps, ProcedureSymbolOps }
...
class Call(val c: whitebox.Context) {
  ...
  def impl(f: Tree): Tree = {
    ...
    q"""
      _root_.com.arkondata.slothql.cypher.syntax.impl.Call.noAlias(
        $procedure,
  ...
  def implAs(outputs: Tree*)(f: Tree): Tree = {
    ...
    val returns = aTrees zip outputs map {
      case (arg, q"scala.Symbol.apply(${name: String})") =>
        ...
        q"_root_.com.arkondata.slothql.cypher.CypherFragment.Return.Expr[_root_.scala.Any]($outExpr, as = $asExpr).known"
    }
    q"""
      _root_.com.arkondata.slothql.cypher.syntax.impl.Call(
        $procedure,
        _root_.scala.List(..$params0Trees),
  ...
  private def mkVar(tpe: Type, name: String): Tree =
    q"_root_.com.arkondata.slothql.cypher.CypherFragment.Expr.Var[$tpe](${name})"
