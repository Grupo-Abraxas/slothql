...
import com.arkondata.slothql.cypher.CypherFragment.Pattern.Rel
import com.arkondata.slothql.cypher.CypherFragment._
import com.arkondata.slothql.util.raiseCompilationError
...
object Match { MatchObj =>
      ...
      f match {
        case Function(params, body) =>
          ...
          val (paramTrees, recTpes) = params.map{ p =>
            ...
            val argTree = q"_root_.com.arkondata.slothql.cypher.CypherFragment.Expr.Param[$tpe](${p.name.decodedName.toString})"
            val recEntry = helper.mkFieldTpe(p.name, tpe)
            argTree -> recEntry
          }.unzip
          val recTpe = helper.mkHListTpe(recTpes)
          val outTypeTree = tq"_root_.com.arkondata.slothql.cypher.CypherFragment.Query[$retType]"

          val query0 = q"$f(..$paramTrees)"
          val query = if (isMatchResult) q"$query0.result" else query0
          q"""
            new _root_.com.arkondata.slothql.cypher.syntax.Match.ParameterizedQuery(
              _root_.com.arkondata.slothql.cypher.CypherFragment.Parameterized[$recTpe, $outTypeTree].apply($query)
            )
           """
        case _ =>
  ...
  class InternalImpl[C <: whitebox.Context](val c: C) {
    ...
    val `syntax pkg` = c.typeOf[com.arkondata.slothql.cypher.syntax.`package`.type]
    ...
    object KnownExpr {
      ...
      def let(name: Name, pattern: Tree): c.Expr[Known[Pattern.Let]] = c.Expr[Known[Pattern.Let]](
        mkKnown(q"""
          _root_.com.arkondata.slothql.cypher.CypherFragment.Pattern.Let(
      ...
      object DeepRel {
        ...
        def length(tree: Tree): Tree = (tree: @unchecked) match {
          ...
          case Apply(tt, List(minT, maxT)) if tt.tpe.resultType =:= `syntax Int-Int` =>
            ...
            def range(ior: Tree) = q"_root_.com.arkondata.slothql.cypher.CypherFragment.Pattern.Rel.Range($ior)"
        ...
        private lazy val relAll = q"_root_.com.arkondata.slothql.cypher.CypherFragment.Pattern.Rel.All"
      }
    }
    ...
    def mkClause[R: c.WeakTypeTag](f: c.Expr[Graph => MatchObj.OptionalResult[R]])(
      ...
      val (pattern, kps, guard) = matchPattern(f)
        ...
        val setAliases = bindSymbols.withFilter(_._2.isDefined).map{
          case (symbol, name) =>
            q"_root_.com.arkondata.slothql.cypher.syntax.Match.Internal.setAlias($symbol, ${name.get.decodedName.toString})"
        }.toList
        ...
        val whereVarExpr0 = c.Expr[Unit](
          q"""
            var $whereVarName: _root_.scala.Option[
              ${ttKnown(tq"_root_.com.arkondata.slothql.cypher.CypherFragment.Expr[_root_.scala.Boolean]")}] = null
           """)
        ...
        val retValExpr = c.Expr[Unit](
          q"""
             val $retName = ${fTransormer.transform(f.tree)}(_root_.com.arkondata.slothql.cypher.syntax.Match.Internal.graph)
           """)
...
//        c.info(c.enclosingPosition, showRaw(res.tree), force = true)
    ...
    private def mkKnown(tpe: Type, tree: Tree) = q"_root_.com.arkondata.slothql.cypher.CypherFragment.Known[$tpe]($tree)"
    private def mkKnown(tree: Tree) = q"_root_.com.arkondata.slothql.cypher.CypherFragment.Known($tree)"
    private def ttKnown(tt: Tree) = tq"_root_.com.arkondata.slothql.cypher.CypherFragment.Known[$tt]"
  }
