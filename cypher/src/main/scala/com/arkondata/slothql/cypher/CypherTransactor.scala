...
object CypherTransactor {
  ...
  def query[F[_]: Applicative, Src, C[_], A](s: CypherStatement.Prepared[A])(implicit
    read: Reader[Src, A]
  ): Tx[F, Src, C, A] =
    FreeT.liftF(Query(s, read))

  def query[F[_]: Applicative, Src, C[_], A](s: CypherStatement.Complete[A])(implicit
    read: Reader[Src, A]
  ): Tx[F, Src, C, A] =
    query(s.withParams(Map()))

  def query[F[_]: Applicative, Src, C[_], A](
    q: CF.Query[A]
  )(implicit gen: CypherStatement.Gen, read: Reader[Src, A]): Tx[F, Src, C, A] =
    query(q.toCypher(gen)._1)

  def query[F[_]: Applicative, Src, C[_], A, Params <: HList](pcq: ParameterizedCypherQuery[Params, A])(implicit
    read: Reader[Src, A]
  ): ParameterizedCypherQuery.Apply[Params, A, Tx[F, Src, C, A]] =
    new ParameterizedCypherQuery.Apply(pcq, query(_))

  def unwind[F[_]: Applicative, Src, C[_]]: C ~> Tx[F, Src, C, *] =
    λ[C ~> Tx[F, Src, C, *]](c => FreeT.liftF(Unwind(c)))
  ...
  def gather[F[_]: Applicative, Src, C[_]: Applicative](implicit
    M: Monad[TxC[F, Src, C, *]]
  ): Tx[F, Src, C, *] ~> TxC[F, Src, C, *] =
    λ[Tx[F, Src, C, *] ~> TxC[F, Src, C, *]](
      _.mapK[TxC[F, Src, C, *]](
        λ[F ~> TxC[F, Src, C, *]] { fa =>
          FreeT.liftT(fa.map(_.pure[C]))
        }
      ).foldMap(
        λ[Operation[Src, C, *] ~> TxC[F, Src, C, *]] { op =>
          FreeT.liftF(mkGather(op))
        }
      )
    )
  ...
  case class Query[Src, C[_], R](query: CypherStatement.Prepared[R], read: Reader[Src, R])
      extends Operation[Src, C, R] {
  ...
  object Gather {
    ...
    def unapply[Src, C[_], R](g: Gather[Src, C, R]): Option[(Operation[Src, C, g.U], C[g.U] => R)] = Some(
      g.op -> g.func
    )
  }
  ...
  def mapOpt[F[_]: Applicative, Src, C[_]: MonoidK, A, B](tx: Tx[F, Src, C, A])(f: A => Option[B]): Tx[F, Src, C, B] =
    tx.flatMap(f(_).map(const[F, Src, C].apply).getOrElse(nothing))

  def flatTraverseTx[F[_]: Applicative, Src, C[_]: Functor, A, B](c: C[A])(
    tx: A => Tx[F, Src, C, B]
  ): Tx[F, Src, C, B] =
    unwindTx[F, Src, C].apply(c.map(tx))

  def traverseTx[F[_]: Applicative, Src, C[_]: Applicative, A, B](c: C[A])(tx: A => Tx[F, Src, C, B])(implicit
    M: Monad[TxC[F, Src, C, *]]
  ): Tx[F, Src, C, C[B]] =
    flatTraverseTx(c)(tx).gather

  def filter[F[_]: Applicative, Src, C[_]: MonoidK, A](tx: Tx[F, Src, C, A])(pred: A => Boolean): Tx[F, Src, C, A] =
    tx.flatMap(a => if (pred(a)) const.apply(a) else nothing)

  def filtering[F[_]: Applicative, Src, C[_]: MonoidK, A](
    tx: Tx[F, Src, C, A]
  ...
  def product[F[_], Src, C[_], A, B](txA: Tx[F, Src, C, A], txB: Tx[F, Src, C, B])(implicit
    S: Semigroupal[Tx[F, Src, C, *]],
    I: Invariant[Tx[F, Src, C, *]]
  ): Tx[F, Src, C, (A, B)] = (txA, txB).tupled
...
//    s"Cannot zip because of different length:\n\t1: $seq1\n\t2: $seq2\n\t3: $seq3")
  ...
  implicit final class SyntaxGatherOps[F[_]: Applicative, Src, C[_]: Applicative, A](tx: Tx[F, Src, C, A])(implicit
    M: Monad[TxC[F, Src, C, *]]
  ) {
    def gather: TxC[F, Src, C, A] = CypherTransactor.gather[F, Src, C].apply(tx)
  }

  implicit final class SyntaxTraverseOps[F[_]: Applicative, Src, C[_]: Applicative, A](ca: C[A])(implicit
    M: Monad[TxC[F, Src, C, *]]
  ) {
  ...
  implicit final class SyntaxOps[F[_]: Applicative, Src, C[_]: MonoidK, A](tx: Tx[F, Src, C, A]) {
  ...
  abstract class Syntax[F[_]: Applicative, Src, C[_]: Applicative: MonoidK](implicit
    txCMonad: Monad[TxC[F, Src, C, *]]
  ) {
    ...
    def query[Params <: HList, R](q: ParameterizedCypherQuery[Params, R])(implicit
      read: Reader[R]
    ): ParameterizedCypherQuery.Apply[Params, R, Tx[R]] = CypherTransactor.query(q)

    def query(q: CF.Query[Nothing])(implicit gen: CypherStatement.Gen): Tx[Unit] = drainGathered(
      CypherTransactor.query(q)
    )
    ...
    def query[Params <: HList](
      q: ParameterizedCypherQuery[Params, Nothing]
    ): ParameterizedCypherQuery.Apply[Params, Unit, Tx[Unit]] =
      new ParameterizedCypherQuery.Apply(
        q.asInstanceOf[ParameterizedCypherQuery[Params, Unit]],
        p => drainGathered(query(p))
      )
