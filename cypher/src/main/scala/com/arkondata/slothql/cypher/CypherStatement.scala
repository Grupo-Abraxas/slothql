...
object CypherStatement {
  ...
  object LiftValue {
    ...
    implicit def liftStringMapValue[T](implicit lift: LiftValue[T]): LiftValue[Map[String, T]] =
      new LiftValue[Map[String, T]] {
    ...
    private lazy val _liftStringMapLiftedValues: LiftValue[Map[String, LiftedValue]] =
      new LiftValue[Map[String, LiftedValue]] {
        def asParam(a: Map[String, LiftedValue]): AnyRef = a.view.mapValues(v => v.lift.asParam(v.value)).toMap.asJava

        def asLiteral(a: Map[String, LiftedValue]): String = literalMap(
          a.view.mapValues(v => v.lift.asLiteral(v.value)).toMap
        )
      }

    private def literalMap(m: Map[String, String]) = m.map { case (k, v) => s"${CypherFragment.escapeName(k)}: $v" }
      .mkString("{", ", ", "}")
  }
  ...
  final case class Part(template: String, params: Map[String, LiftValue[_]]) extends CypherStatement {
    ...
    def copy(template0: String, params0: Map[String, LiftValue[_]]): this.type =
      Part(template0, params0).asInstanceOf[this.type]
  }

  final case class Complete[+T](template: String, params: Map[String, LiftValue[_]]) extends CypherStatement {

    def copy(template0: String, params0: Map[String, LiftValue[_]]): this.type =
      Complete(template0, params0).asInstanceOf[this.type]

    def withParams(params: Map[String, Any]): Prepared[T @uncheckedVariance] =
      withParamsUnchecked(params)
        .ensuring(
          this.params.keys == params.keys,
  ...
  object Gen {
    ...
    final class Default protected (
      ...
      def nextAlias(alias: Alias): (String, Gen) =
        hashedAliases
          ...
          .getOrElse {
            ...
            val s1     = s"$s$suffix"
            s1 -> new Default(
              hashedAliases.updated(alias.##, s1),
              usedAliases.updated(s, suffix + 1),
              hashedParams,
              usedParams
            )
          }
      ...
      def nextParam(param: Param): (String, Gen) = () match {
        ...
        case _ =>
          param.name -> new Default(hashedAliases, usedAliases, hashedParams + param.##, usedParams + param.name)
      }
    }
  ...
  object GenF {
    ...
    def sequence[C[_]: Applicative: Foldable, A](fs: C[GenF[A]])(implicit M: MonoidK[C]): GenF[C[A]] = new GenF(
      fs.foldRight(Eval.now((M.empty[A], _: Gen))) { case (f, accF) =>
        accF.map(_ andThen { case (acc, gen) =>
          f.f andThen Arrow[Function1].first(_.pure[C] <+> acc) apply gen
        })
      }.value
    )
  }
  ...
  object GenS0 {
    ...
    implicit class GenSTuple2Ops[S <: CypherStatement](gens: (GenS[S], GenS[S]))(implicit mk: MkStatement[S]) {
      ...
      def map2(f: (String, String) => String): GenS[S] = GenS(
        (gens._1.f, gens._2.f).mapN((s1, s2) => mk(f(s1.template, s2.template), s1.params ++ s2.params))
      )
    }
  }
  ...
  object GenS {
    ...
    def partsSequence[C[_]: Applicative: Foldable](fs: C[_ <: CypherFragment.Aux[Part]])(implicit
      M: MonoidK[C]
    ): GenS0[C, Part] =
      sequence(fs.map(part))

    def completeSequence[A]: CompleteSequenceBuilder[A] =
      CompleteSequenceBuilder.asInstanceOf[CompleteSequenceBuilder[A]]

    protected class CompleteSequenceBuilder[A] {

      def apply[C[+_]: Applicative: Foldable](fs: C[CypherFragment.Aux[Complete[_]]])(implicit
        M: MonoidK[C]
      ): GenS0[C, Complete[A]] =
    ...
    def liftAlias(opt: Option[Alias]): GenS[Part] = opt.map {
  ...
  implicit class GenSPartOps[C[_]: Functor: Foldable](gf: GenS0[C, Part]) {
    ...
    def toComplete[T]: GenS0[C, Complete[T]] = GenS0(gf.f.map(_.map { case Part(template, params) =>
      Complete[T](template, params)
    }))
  }
