...
class Neo4jCypherTransactorTest extends AnyWordSpec with Matchers with Neo4jUsingTest {
  ...
  private lazy val allVertices = Seq(
    ...
    GraphElem.Node(0, List("Group"), Map("name" -> "Root Group", "id" -> "g1")),
    GraphElem.Node(0, List("Group"), Map("name" -> "Sub Group", "id" -> "g2")),
  ...
  "Neo4jCypherTransactor" should {
    ...
    "assign paths to variables" in {
      val query = tx.query(Match { case ps ::= (Node("Group") - Rel(0 ** _) > Node("User")) =>
        (
          ps.length,
      ...
      test[(Long, List[Map[String, Any]], List[String])](
        query,
        Seq(
          (
            2,
    ...
    object ChainAndGatherTest {
      ...
      def groupDepQuery(id: String) = Match {
        case (g @ Node("Group")) - _ > _ - _ > u if u.prop[String]("id") === lit(id) =>
          g.props orderBy g.prop[String]("id")
      }

      val groupQuery = Match { case g @ Node("Group") => g.prop[String]("id") }
      def userDepQuery(id: String) = Match {
        case (u @ Node("User")) < _ - _ < _ - g if g.prop[String]("id") === lit(id) =>
          u.props orderBy u.prop[String]("id")
      }

      def groupByIdQuery(id: String) = Match { case g @ Node("Group", "id" := `id`) =>
        g.props orderBy g.prop[String]("id")
      }
    }
    ...
    "`gather` query results to Vector" in {
      ...
      test[(String, Vector[Map[String, Any]])](
        query,
        Seq(
          "u1" -> Vector(
            Map("name" -> "Root Group", "id" -> "g1"),
    ...
    "`traverseTx` streams" in {
      ...
      test[(Vector[String], Vector[Map[String, Any]])](
        query,
        Seq(
          Vector("g1", "g2") -> Vector(
            Map("name" -> "John", "email" -> "john@example.com", "confirmed" -> true, "age" -> 28, "id" -> "u1"),
    ...
    "filter results with another read transaction" in {
      ...
      def predTx(id: String) = tx query Match { case g @ Node("id" := `id`) =>
        g.prop[String]("name") contains lit("Root")
      }
    ...
    "`zip` results of two queries" in pendingUntilFixed {
      ...
      val q1 = tx.query(Match { case g @ Node("Group") =>
        g.prop[String]("name")
