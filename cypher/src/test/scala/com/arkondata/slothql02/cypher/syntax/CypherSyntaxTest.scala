...
import com.arkondata.slothql02.neo4j.Neo4jCypherTransactor.RecordReader
...
class CypherSyntaxTest extends WordSpec with Matchers {
  ...
  "Slothql cypher syntax" should {
    ...
    "assign paths to variables (2)" in test(
      Match {
        case ps ::= (Vertex("Group") - e > v - _ *:(0 - _, Edge("foo", "bar")) > (g@Vertex("Group"))) =>
          (
            ps.nodes.map(_.props),
            ps.edges.map(_.props),
            ps.length,
    ...
    "stored procedures calls (void output)" in test(
    ...
    "support `exists` predicate" in test(
