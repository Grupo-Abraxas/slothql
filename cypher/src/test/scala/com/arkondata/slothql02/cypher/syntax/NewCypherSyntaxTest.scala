...
//        """value asString is not a member of com\.arkondata\.slothql02\.cypher\.CypherFragment\.Expr\.MapKey\[List\[Int\]\]"""
//      )
...
*/
    ...
    "support simple CASE expressions (without default)" in pending /*test(
      Match { case v =>
    ...
    "support simple CASE expressions (with default)" in pending /*test(
      Match { case v =>
    ...
    "support generic CASE expressions (without default)" in pending /*test(
      Match { case v =>
    ...
    "support generic CASE expressions (with default)" in pending /*test(
      Match { case v =>
